...
# ========================================================================
# SHARED CONFIGURATION (RSS specific)
# ========================================================================

RSS_ROOT = Path(r'../Research/RSS')

# RSS Configuration
RSS_CONFIG_PATH = Path('rss_feeds.json')
RSS_PROCESSED_LOG_NAME = '_processed_articles.json'
RSS_MAX_ARTICLES_PER_FEED = 20
RSS_REQUEST_DELAY_SECONDS = 2
RSS_USER_AGENT = 'Mozilla/5.0 (RSS Research Collector)'
...
def load_rss_config(config_path: Path) -> List[Dict]:
...
def load_rss_processed_ids(provider_folder: Path) -> set:
    """Load the set of previously saved article IDs for a provider."""
    log_path = provider_folder / RSS_PROCESSED_LOG_NAME
...
def save_rss_processed_ids(provider_folder: Path, processed_ids: Iterable[str]) -> None:
...
def save_article(provider: str, feed_name: str, entry: Dict, provider_folder: Path) -> Path:
    ...
    print(f"  [DEBUG] Saving to: {filename.resolve()}")
...
*Saved using Unified Content Scraper*
"""
...
def fetch_feed(url: str) -> feedparser.FeedParserDict:
    ...
    try:
        response = requests.get(url, headers={'User-Agent': RSS_USER_AGENT}, timeout=30)
...
def process_rss_feed(provider: str, feed: Dict, processed_ids: set, provider_folder: Path) -> Dict[str, int]:
    ...
    entries = parsed.entries[:RSS_MAX_ARTICLES_PER_FEED]
    if not entries:
...
def process_rss_provider(provider_cfg: Dict) -> None:
    ...
    processed_ids = load_rss_processed_ids(provider_folder)
    before_count = len(processed_ids)

    totals = {'downloaded': 0, 'skipped': 0, 'errors': 0}

    for feed in feeds:
        stats = process_rss_feed(provider_name, feed, processed_ids, provider_folder)
        for key, value in stats.items():
            totals[key] += value
        time.sleep(RSS_REQUEST_DELAY_SECONDS)

    if len(processed_ids) != before_count:
        save_rss_processed_ids(provider_folder, processed_ids)

    print(f"  Summary for {provider_name}: {totals['downloaded']} new, {totals['skipped']} skipped, {totals['errors']} errors")


def run_rss_scraper():
    """Run the RSS article scraper"""
    print("\n" + "=" * 60)
    print("RSS ARTICLE SCRAPER")
    print("=" * 60)

    if not RSS_CONFIG_PATH.exists():
        print("\nWARNING: rss_feeds.json not found - skipping RSS scraping")
        return None

    try:
        config = load_rss_config(RSS_CONFIG_PATH)
    except Exception as exc:
        print(f'\nERROR: {exc}')
        return None

    if not config:
        print('\nNo providers configured in rss_feeds.json - skipping RSS scraping')
        return None
    ...
    for provider_cfg in config:
        try:
            process_rss_provider(provider_cfg)
            providers_count += 1
            # Approximate articles per provider
            total_articles += RSS_MAX_ARTICLES_PER_FEED
        except Exception as exc:
            print(f"\n[ERROR] Unexpected failure for {provider_cfg.get('provider', 'Unknown Provider')}: {exc}")
            continue

    print(f'\nRSS articles saved to: {RSS_ROOT}')
...
if __name__ == "__main__":
    run_rss_scraper()
    print("\n[OK] RSS Scraping complete! Closing in 3 seconds...")
    time.sleep(3)
